...
class UpdateTemplateRequest(BaseModel):
    ...
    - category_slug es EDITABLE (slug del catálogo dinámico, RN-CAT11).
    ...
    def at_least_one_mutable_field(self) -> "UpdateTemplateRequest":
        mutable = [self.label, self.is_safe, self.paths, self.category_slug]
        if all(v is None for v in mutable) and \
                "origin_template_id" not in (self.model_fields_set or set()):
            raise ValueError(
                "El body debe contener al menos uno de: label, category_slug, origin_template_id, is_safe, paths."
            )
        return self
...
async def _clone_template_to_world(
    ...
    dest = await noise_db.create_destination(
        ...
        frequency_weight=navigation_weight,   # peso por-mundo (v2 — ya no en la plantilla)
...
async def list_templates(
    ...
    Filtros: category_slug (slug del catálogo), include_paths (boolean), limit, offset.
...
async def update_template(
    ...
    try:
        updated = await rt_port.update_template(
            ...
            label=body.label,
            is_safe=body.is_safe,
            category_slug=body.category_slug,  # None = no cambiar; str = actualizar
